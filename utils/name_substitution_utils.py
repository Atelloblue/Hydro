...
REPLACEMENT_REGEXES_STR = [
    ...
    (r'(?:Google )?Chrom(e|ium)(?!\w)', r'Hydro'),
...
def replace_text(text):
    """Replaces instances of Chrom(e | ium) with Hydro in strings"""
    had_match = False
...
def replace_grit_message(msg):
    """
    Replaces instances of Chrom(e | ium) with Hydro
...
def replace_xtb_translation(msg, fp_map):
    """
    Replaces instances of Chrom(e | ium) with Hydro in a
    .xtb <translation>.
    """
...
def replace_grit_tree(text):
    """Replaces instances of Chrom(e | ium) with Hydro, where desired"""
...
def dedup_translations_in_place(translation, seen):
    ...
    # "Chrome" variant. Since we substituted both of those with Hydro,
